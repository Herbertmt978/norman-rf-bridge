...
## Current implementation: 0.10.0 experimental
...
| `LearningSession` | Explicitly owned, receive-only capture with ambiguity checks, distinct presses and expiry; no standalone transmit or persistence authority |
| Profile management | Identity-checked rename/removal; a reusable tombstone stays within the existing record layout, with no global NVS erase |
...
Version 0.10.0 schedules up to two additional unchanged direct-command bursts,
with a two-second gap after each successful completion, within60seconds of the
original start. Version0.9.2 used20-second start intervals.
